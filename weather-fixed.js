...
function updateForecast() {
  ...
  (async () => {
    try {
      ...
      const weatherURL = `https://api.open-meteo.com/v1/forecast?latitude=${lat}&longitude=${lon}&daily=temperature_2m_min,temperature_2m_max,cloudcover_mean,precipitation_sum,windspeed_10m_max&temperature_unit=celsius&windspeed_unit=kmh&timezone=auto&start_date=${today}&end_date=${endDate}`;
...
function getDailyWeatherIcons(cloudcover, precipitation) {
  let icons = [];
  
  // Add icons based on weather conditions
  // Rain icon if there's precipitation
  if (precipitation > 0) {
