...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (cityInput && citySuggestions) {
    ...
    citySuggestions.style.zIndex = "1100"; // Ensure it appears above other elements
    ...
    if (window.innerWidth <= 480) {
      citySuggestions.style.maxHeight = "180px";
    }
      ...
      try {
        ...
        if (data.results) {
          ...
          placesWithDistance
            ...
            suggestion.style.padding = "12px 15px"; // Increased touch target
            ...
            // Make suggestions more touch-friendly on mobile
            if (window.innerWidth <= 768) {
              suggestion.style.padding = "14px 15px"; // Even larger touch target on mobile
              suggestion.style.fontSize = "16px"; // Larger text for readability
            }
            ...
            suggestion.addEventListener("click", () => {
              console.log("DEBUG: City suggestion clicked - updating inputs only");
              ...
              // Allow user to click the loadMap button manually
              // Removed auto-click to prevent immediate map refresh
            });
    ...
    cityInput.addEventListener("keydown", (event) => {
      if (event.key === "Enter") {
        console.log("DEBUG: Enter key pressed in city input");
        const suggestions = citySuggestions.querySelectorAll("div");
        if (suggestions.length === 0) return;
        ...
        if (selectedSuggestionIndex !== -1) {
          ...
          if (selectedSuggestion.dataset.latitude && selectedSuggestion.dataset.longitude) {
            ...
            document.getElementById("selected-city").textContent = `${selectedLocationText}: ${cityName}`;
            
            // Save location to localStorage
            const country = selectedSuggestion.textContent.split(',')[1]?.trim();
            saveLocationToLocalStorage(cityName, lat, lon, country);
            
            // Show map container but don't load map automatically
    ...
    citySuggestions.addEventListener("mousedown", (event) => {
      console.log("DEBUG: Suggestion mousedown event triggered");
      const suggestions = citySuggestions.querySelectorAll("div");
      suggestions.forEach((suggestion, index) => {
    ...
    // Load last location data only (without auto-refreshing map)
    console.log("DEBUG: Setting up setTimeout for loading last location data only");
    setTimeout(() => {
      // Make sure language is loaded first
      if (typeof _t === 'function') {
        console.log("DEBUG: Translation function available, loading location data only");
        // Modified to just load location data without auto-triggering map refresh
        const locationData = localStorage.getItem('lastLocation');
        if (locationData) {
          try {
            const location = JSON.parse(locationData);
            if (location.city && location.latitude && location.longitude) {
              ...
              // Just update the selected city text without refreshing map
              document.getElementById("selected-city").textContent =
                `${_t('selectedLocation')}: ${label}`;
              
              console.log("DEBUG: Loaded previous location data without auto-refreshing map");
            }
          } catch (e) {
            console.error("Error parsing saved location:", e);
          }
        ...
        setTimeout(() => {
          // Same change as above - just load data without auto-refreshing map
          const locationData = localStorage.getItem('lastLocation');
          if (locationData) {
            try {
              const location = JSON.parse(locationData);
